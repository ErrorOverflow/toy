...
def func_residual_unit(data, num_filter, stride, dim_match, name):
    ...
    conv1 = layers.conv2d(data = act1, channels = int(num_filter), groups = groups, kernel_size = kernel_size1, strides = stride, padding = padding1, data_layout = NCHW, kernel_layout = OIHW, name = name_conv1)
    name_bn2 = "_bn2"
    name_bn2 = name + name_bn2
    bn2 = layers.batch_norm_infer(data = conv1, epsilon = epsilon, scale = symbol_true, name = name)
...
def func_main():
    ...
    while i < num_stages:
        ...
        j_range = units[i]
        tmp5 = 1
        j_range = j_range - tmp5
        stride2 = (1,1)
        j = 0
        while j < j_range:
            tmp7 = 1
            filter_list_index2 = i + tmp7
